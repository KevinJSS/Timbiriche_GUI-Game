...
public class GameBoard {
        ...
        for (int r = 0; r < board.length; r++) {
            for (int c = 0; c < board[r].length; c++) {
                if ((r % 2) == 0) {
                    ...
                    if ((c % 2) != 0) {
                        board[r][c] = "  " + ((num <= 9) ? ("0" + String.valueOf(num)) : String.valueOf(num));
                        num++;
                    } else {
    ...
    public boolean isAvailable(int row, int column) {
        return board[row][column].equals(BLANK_SPACE);
    }

    public int[] nearQuadrant(int row, int column) {
    ...
    public int fillFullQuadrants(String playerSymbol) {
        int scoreCount = 0;
        int[] quadrant;
        for (int i = 0; i < vectorPositions.length; i++) {
            quadrant = findNumberPosition("  " + ((i <= 9) ? "0" + String.valueOf(vectorPositions[i])
                    : String.valueOf(vectorPositions[i])));
            if (quadrant != null && fullQuadrant(quadrant)) {
                System.out.println("SI ENTRE");
                setQuadrantSymbol(quadrant, playerSymbol);
                scoreCount++;
            }
        }
        return scoreCount;
    ...
    public void addLine(int row, int column) {
        board[row][column] = lineType(row);
    }

    public String lineType(int row) {
        if ((row % 2) == 0) {
            return HORIZONTAL_LINE;
        } else {
            return VERTICAL_LINE;
